...
void send_date_and_time_to_client(server_t *server) {
  ...
  size_t bytes_written = write(server->data_socket_fd, time_as_str,
                               bytes_to_write);
...
void send_unkown_command_to_client(server_t *server) {
  ...
  size_t bytes_written = write(server->data_socket_fd, unknown_command_msg,
                               bytes_to_write);
...
void send_content_of_file_to_client(server_t *server, char *file_path) {
  ...
  size_t bytes_read = read(file_fd, file_content_buffer, file_stat.st_size);

  if (bytes_read < 0) {
    fprintf(stderr, "Error occured in read(): %s!\n", strerror(errno));
  }

  size_t bytes_written = write(server->data_socket_fd, file_content_buffer,
                              file_stat.st_size);
...
void send_open_error_to_client(server_t *server) {
  ...
  size_t bytes_written = write(server->data_socket_fd, open_error_msg,
                               bytes_to_write);
